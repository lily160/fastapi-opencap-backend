...
from config.settings import (
    ...
    FORGOT_LOOKUP_EXPIRE, FORGOT_MAX_RETRY,
)
...
from config.settings import KAFKA_FORGOT_PASSWORD_TOPIC
...
async def refresh_access_token(db: AsyncSession, refresh_token: str) -> TokenResponse:
    ...
    result = await db.execute(select(TokenBlacklist).where(
        TokenBlacklist.token_hash == token_hash(refresh_token), TokenBlacklist.token_type == "refresh",
    ))
...
async def revoke_refresh_token(db: AsyncSession, refresh_token: str) -> None:
    ...
    result = await db.execute(select(TokenBlacklist).where(
        ...
        TokenBlacklist.token_type == "refresh", TokenBlacklist.revoked_at.is_(None),
    ))
    stored_token = result.scalars().first()
    # 如果 Token 存在，则标记为已撤销
    if stored_token:
...
async def revoke_all_user_refresh_tokens(db: AsyncSession, user_id: str) -> None:
    ...
    result = await db.execute(select(TokenBlacklist).where(
        TokenBlacklist.user_id == user_id, TokenBlacklist.token_type == "refresh",
...
async def revoke_access_token(db: AsyncSession, user_id: str, payload: dict) -> None:
...
async def get_valid_forgot_session(db: AsyncSession, forgot_id: str) -> ForgotPasswordSession:
    # 根据 forgot_id 查询找回密码会话
    session = await db.get(ForgotPasswordSession, forgot_id)
    # 会话不存在或已被使用
...
async def lookup(request: ForgotPasswordRequest, db: AsyncSession) -> ForgotLookupResponse:
    result = await db.execute(select(User).where(User.username == request.username.strip()))
    user = result.scalars().first()
    if not user or not user.is_active:
...
async def send_forgot_code(payload: ForgotSendCodeRequest, db: AsyncSession) -> ForgotSendCodeResponse:
    # 获取有效的找回密码会话
    forgot_session = await get_valid_forgot_session(db, payload.forgot_id)
    # 查询用户
    user = await db.get(User, forgot_session.user_id)
    # 用户不存在或已禁用
    if not user or not user.is_active:
        raise HTTPException(
...
async def reset_forgot_password(payload: ForgotResetRequest, db: AsyncSession) -> None:
    # 获取有效的找回密码会话
    session = await get_valid_forgot_session(
        db,
        payload.forgot_id
    )
    # 检查验证码是否已经发送
...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db)) -> TokenResponse:
    return await issue_tokens(db, await authenticate_user(db, payload.username, payload.password))
...
async def logout(payload: RefreshRequest | None = None, auth_context: AuthContext = Depends(get_auth_context),
                 db: AsyncSession = Depends(get_db)) -> Response:
    if payload and payload.refresh_token:
        await revoke_refresh_token(db, payload.refresh_token)
    else:
        await revoke_all_user_refresh_tokens(db, auth_context.user.user_id)
    await revoke_access_token(db, auth_context.user.user_id, auth_context.payload)
    return Response(status_code=CODE_NO_CONTENT)


@router.put("/password", status_code=CODE_NO_CONTENT)
async def change_password(payload: PasswordChangeRequest, auth_context: AuthContext = Depends(get_auth_context),
                          db: AsyncSession = Depends(get_db)) -> Response:
    ...
    await revoke_all_user_refresh_tokens(db, user.user_id)
    await revoke_access_token(db, user.user_id, auth_context.payload)
    await db.commit()
    return Response(status_code=CODE_NO_CONTENT)


@router.post("/forgot/lookup", response_model=ForgotLookupResponse, status_code=CODE_SUCCESS)
async def forgot_lookup(payload: ForgotPasswordRequest, db: AsyncSession = Depends(get_db)) -> ForgotLookupResponse:
    return await lookup(payload, db)


@router.post("/forgot/send-code", response_model=ForgotSendCodeResponse, status_code=CODE_SUCCESS)
async def forgot_send_code(payload: ForgotSendCodeRequest, db: AsyncSession = Depends(get_db)) -> ForgotSendCodeResponse:
    return await send_forgot_code(payload, db)


@router.post("/forgot/reset", status_code=CODE_SUCCESS)
async def forgot_reset(payload: ForgotResetRequest, db: AsyncSession = Depends(get_db)) -> dict[str, str]:
    await reset_forgot_password(payload, db)
